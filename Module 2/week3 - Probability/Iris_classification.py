...
def create_train_data_iris():
  data = np.loadtxt(r"C:\Users\Administrator\Desktop\AIO-Exercise\Module 2\week3 - Probability\iris.data.txt", delimiter=",", dtype=str)
  return data
...
def compute_prior_probablity_iris(train_data):
  ...
  for i in range(0,len(y_unique)):
    prior_probability[i]=len(np.nonzero(train_data[:,-1] == y_unique[i])[0])/len(train_data)
...
def compute_conditional_probability_iris(train_data):
  ...
  for i in range(0,train_data.shape[1]-1):
    ...
    for j in range(0,len(y_unique)):
        mean = np.mean((train_data[:,i][np.nonzero(train_data[:,-1] == y_unique[j])]).astype(float))
        sigma =  np.std((train_data[:,i][np.nonzero(train_data[:,-1] == y_unique[j])]).astype(float))
        sigma_square = sigma * sigma
        x_conditional_probability[j]= [mean, sigma_square]
...
def prediction_iris(_,  prior_probability, conditional_probability):
